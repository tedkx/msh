...
const debug = createDebug('SagaFlow');
...
export const createLinkedActions = (actionName, clear) => {
  ...
  if (clear === true) {
    ...
    const clearKey = actionParts.length === 1
      ? 'CLEAR_' + actionName
      : actionParts.map((part, i) => (i === 0
        ? 'CLEAR'
        : part)).join('_');
...
export const extractSagaError = (exception, presetErrorMessage) => {
  ...
  const hasResponse = !!exception && !!exception.response,
    responseData = hasResponse
      ? exception.response.data
      : {},
    statusCode = exception.response && exception.response.status
  ...
  return {
    ...
    message: presetErrorMessage || (responseData.Messages && responseData.Messages[0] && responseData.Messages[0].Description) || exception.message,
  ...
  return takeEvery(actionName, function * defaultSagaFlow(action) {
    try {
      ...
      if (isFunc(options.successMessage)) 
        yield put(notify(options.successMessage.apply(null, [content]), 'Success', 5000));
      else if (!isNilOrWhiteSpace(options.successMessage)) 
        yield put(notify(options.successMessage, NotificationTypes.Success, 5000));
      
      yield put(createAction(`${actionName}_SUCCESS`, content));
    } catch (e) {
      let err = extractSagaError(e, options.errorMessage);
      debug('saga error', e, '->', err);
      if (!options.suppressNotification) 
        yield put(notify(err.message, NotificationTypes.Error));
      yield put(createAction(`${actionName}_FAIL`, null, err.message));
    }
  });
};
