...
module.exports = {
  ...
  resolve: {
    ...
    alias: {
      ...
      dummyAssets: path.resolve(paths.appSrc, './assets/dummy'),
    ...
    rules: [
      ...
      // before Babel processes the JS.
      {
        ...
        oneOf: [
          ...
          {
            test: [
              /\.bmp$/, /\.gif$/, /\.jpe?g$/, /\.png$/, /\.(eot|woff|woff2|otf)/
            ],
          ...
          // INTERNAL CSS "postcss" loader applies autoprefixer to our CSS. "css" loader
          // resolves paths in CSS and adds assets as dependencies. "style" loader turns
          // CSS into JS modules that inject <style> tags. In production, we use a plugin
          // to extract that CSS to a file, but in development "style" loader enables hot
          // editing of CSS.
          {
            test: /\.css$/,
            exclude: /(node_modules)|(semantic\.min\.css)/,
            use: [
          ...
          {
            test: /\.css$/,
            include: /(node_modules)|(semantic\.min\.css)/,
            //exclude: /src/,
            use: [
              require.resolve('style-loader'), {
                loader: require.resolve('css-loader'),
